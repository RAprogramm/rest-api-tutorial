...
func (d *db) Create(ctx context.Context, user user.User) (string, error) {
	...
	d.logger.Trace(user)
	return "", fmt.Errorf("failed to convert oid to hex. probably oid: %s", oid)
}
...
func (d *db) FindOne(ctx context.Context, id string) (u user.User, err error) {
	...
	if err != nil {
		return u, fmt.Errorf("failed to convert hex to ObjectID. hex is: %s", id)
	}
...
func (d *db) Update(ctx context.Context, user user.User) error {
	...
	if err != nil {
		return fmt.Errorf("failed to convert user ID(%s) to objectID", user.ID)
	}

	fileter := bson.M{"_id": objectID}
	...
	result, err := d.collection.UpdateOne(ctx, fileter, update)
	if err != nil {
